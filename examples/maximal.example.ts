...
export const invoiceDepot = korm.depots.s3({
  bucket: "invoices",
...
const walDepot = korm.depots.local(walDepotPath);
...
const pool = korm
  ...
  .setLayers(
    ...
    korm.use.layer(importantDb).as("importantDb"),
  )
  ...
  .withLocks(korm.target.layer("userdb"))
  .setDepots(
  ...
  .withMeta(korm.target.layer("userdb"))
  ...
  .retain(10)
  .days() // retain all backups younger than 10 days, discard older
...
const newUser = (
  await korm
    ...
    .create()
) // This creates all needed tables with the correct columns to hold this item or inserts it if tables exist and have the correct shape
...
const updatedCar = (
  await newCar
    .update(changeYear(newCar.data!, 2010)) // Update takes a DeepPartial<T>, in this case DeepPartial<Car> and produces an UncommittedItem<T>, an Item that exists in the DB but is out of sync here. It only has the .commit() method.
    .commit()
) // This saves an uncommited item to the source layer and returns an Item<T>
  .unwrap();
...
const userWantsLogin = await korm
  .item<User>(pool)
  .from.query(korm.rn("[rn][from::userdb]:users:freetier:*"))
  .where(eq("username", incomingUsername))
  .get();
...
const testResult =
  await userWantsLogin[0]?.data?.password.verifyPassword(incomingPassword);
if (!testResult) {
  // 401 -> Wrong password
} else {
...
const carUpdates = fredsCars.map((car) =>
  car.update({ registered: false }).unwrap(),
);
...
const newWarning = korm
  .item<UserWarning>(pool)
  .from.data({
    mods: [korm.mods.from("importantDb")],
    namespace: "warnings",
    kind: "financial",
    data: {
      user: fredRN,
      warningText:
        "You have unpaid fees. To re-register your cars, please update your payment method.",
    },
  })
  .unwrap();
...
const theCarOwnerUpdate = theCar
  .update({ owner: { lastName: "Simpson" } })
  .unwrap();
...
const invoicePdf = korm.file({
  ...
  file: new Blob(["Invoice for Fred Flintstone - total: 199"], {
    type: "text/plain",
  }),
