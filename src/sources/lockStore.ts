...
export class LayerLockStore implements LockStore {
    ...
    private async _ensureLocked(): Promise<void> {
        ...
        throw new Error(`Lock store does not support layer type "${this._layer.type}".`);
    }

    async tryAcquire(lockId: string, ownerId: string, ttlMs: number): Promise<boolean> {
        await this.ensure();
        const now = Date.now();
    ...
    async refresh(lockId: string, ownerId: string, ttlMs: number): Promise<boolean> {
        await this.ensure();
        const now = Date.now();
        const expiresAt = now + ttlMs;
        const normalized = normalizeLockId(lockId);
