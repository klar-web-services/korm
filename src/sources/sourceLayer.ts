...
import type { BackupContext, BackupPayload, BackupRestoreOptions } from "./backups";
...
export interface SourceLayer {
    ...
    insertItem<T extends JSONable>(item: FloatingItem<T>, options?: PersistOptions): Promise<DbChangeResult<T>>;
    /** Update an existing item and return a change result. */
    updateItem<T extends JSONable>(item: UncommittedItem<T>, options?: PersistOptions): Promise<DbChangeResult<T>>;
    ...
    executeQuery<T extends JSONable>(query: QueryBuilder<T>): Promise<Result<Item<T>[]>>;
    ...
    getColumnKinds(namespace: string, kind: string): Promise<Map<string, ColumnKind>>;
    ...
    restore?: (payload: BackupPayload, options?: BackupRestoreOptions) => Promise<void> | void;
