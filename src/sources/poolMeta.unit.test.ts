...
const makeTempDir = (): string => fs.mkdtempSync(path.join(os.tmpdir(), "korm-meta-"));

const makeSqlitePath = (): string => path.join(makeTempDir(), "pool.sqlite");

const withEncryptionKey = async (key: string, run: () => Promise<void>): Promise<void> => {
...
describe("pool metadata", () => {
    test("allows matching config across instances", async () => {
        ...
        const pool1 = korm.pool()
            .setLayers({ layer: layer1, ident: "db" })
            .withMeta({ layerIdent: "db" })
            .open();
        await pool1.ensureWalReady();

        const layer2 = korm.layers.sqlite(dbPath);
        const pool2 = korm.pool()
            .setLayers({ layer: layer2, ident: "db" })
            .withMeta({ layerIdent: "db" })
            .open();
    ...
    test("discovers pool configuration from metadata", async () => {
        ...
        const pool = korm.pool()
            .setLayers({ layer, ident: "db" })
            .setDepots({ depot, ident: "files" })
            .withMeta({ layerIdent: "db" })
            .open();
    ...
    test("danger.reset clears pool metadata", async () => {
        ...
        const pool2 = korm.pool()
            .setLayers({ layer: layer2, ident: "db" })
            .withMeta({ layerIdent: "db" })
            .withLocks({ layerIdent: "db", ttlMs: 5_000 })
            .open();
        await pool2.ensureWalReady();
        await pool2.close();
    });
    ...
    test("serializes complex config options for discovery", async () => {
        await withEncryptionKey("b7b6d2f594f1f4a4f2f6f10aeac69547fd0b9cbd398081b62b68a26252c5bb22", async () => {
            ...
            const layers = new Map<string, SourceLayer>([
                ["db", {
                    type: "pg",
                    identifier: "pg@test",
                    getPoolConfig: () => ({ type: "pg", mode: "options", value: options }),
            ...
            const hydratedLayer = hydrated.layers.find((layer) => layer.ident === "db");
            ...
            expect(Buffer.from(hydratedOptions.ssl.ca).toString("utf8")).toBe("secret");
            expect(hydratedOptions.ssl.key).toBeInstanceOf(Uint8Array);
            expect(Array.from(hydratedOptions.ssl.key as Uint8Array)).toEqual([1, 2, 3]);
            expect(hydratedOptions.ssl.cert).toBeInstanceOf(Float32Array);
            expect(Array.from(hydratedOptions.ssl.cert as Float32Array)).toEqual([1.25, 2.5]);
            ...
            expect((hydratedOptions.ssl.when as Date).toISOString()).toBe("2024-01-01T00:00:00.000Z");
            expect(hydratedOptions.ssl.endpoint).toBeInstanceOf(URL);
            expect((hydratedOptions.ssl.endpoint as URL).toString()).toBe("https://example.com/");
            expect(hydratedOptions.ssl.map).toBeInstanceOf(Map);
            expect((hydratedOptions.ssl.map as Map<unknown, unknown>).get("a")).toBe(1);
    ...
    test("rejects functions in config options", async () => {
        await withEncryptionKey("6f0725a767f2ed27e7c2e6f114f9304318e1542b8a64ae6cd43194a8fc8d7150", async () => {
            const layers = new Map<string, SourceLayer>([
                ["db", {
                    type: "pg",
                    identifier: "pg@test",
