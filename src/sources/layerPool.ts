...
export class LockTimeoutError extends Error {
    constructor(rn: RN<any>, timeoutMs: number) {
        super(`Lock acquisition timed out after ${timeoutMs}ms for RN "${rn.value()}"`);
...
export class KormLocker {
    ...
    private async _acquireShared(rn: RN<any>, timeoutMs: number): Promise<() => void> {
        if (!this._shared) {
            return () => {};
        }
     ...
     */
    async acquire(rn: RN<any>, timeoutMs: number = DEFAULT_LOCK_TIMEOUT_MS): Promise<() => void> {
     ...
     */
    async acquireMultiple(rns: RN<any>[], timeoutMs: number = DEFAULT_LOCK_TIMEOUT_MS): Promise<() => void> {
...
export class LayerPool {
     ...
     */
    constructor(layers: Map<string,SourceLayer>, depots: Map<string, Depot> = new Map(), options?: LayerPoolOptions) {
        this._layers = layers;
        this._depots = depots;
    ...
    private _initLocks(lockMode: LockMode): KormLocker {
        ...
        if (!layer) {
            ...
            throw new Error(`Lock layer "${lockMode.layerIdent}" is not present in the pool. Available layers: ${available}`);
        }
        const store = new LayerLockStore(layer);
        const ownerId = lockMode.ownerId ?? `${this._ident}-${crypto.randomUUID().slice(0, 8)}-${process.pid ?? "pid"}`;
        const ttlMs = lockMode.ttlMs ?? DEFAULT_LOCK_TTL_MS;
        const retryMs = lockMode.retryMs ?? DEFAULT_LOCK_RETRY_MS;
        const refreshMs = lockMode.refreshMs ?? Math.max(MIN_LOCK_REFRESH_MS, Math.floor(ttlMs / 2));
        const shared: SharedLockConfig = { store, ownerId, ttlMs, retryMs, refreshMs };
        ...
        this._metaReady = (async () => {
            ...
            if (existing.length === 0) {
                if (options?.metaMode) {
                    ...
                    if (!target) {
                        const available = Array.from(this._layers.keys()).join(", ");
                        throw new Error(`Pool meta layer "${options.metaMode.layerIdent}" is not present in the pool. Available layers: ${available}`);
                    }
            ...
            for (const entry of existing) {
                comparePoolMetaConfig(config, entry.config).forEach((item) => discrepancies.add(item));
            }
            if (existing.length > 1) {
                discrepancies.add(`pool meta present in multiple layers: ${existing.map((entry) => entry.ident).join(", ")}`);
            }
    ...
    private _startBackups(): void {
        ...
        if (!metaLayer) {
            const available = Array.from(this._layers.keys()).join(", ");
            throw new Error(`Pool meta layer "${metaIdent}" is not present in the pool. Available layers: ${available}`);
        }
    ...
    async close(): Promise<void> {
        ...
        const depotCloses = Array.from(this._depots.values()).map((depot) => {
            const maybeClose = depot as Depot & { close?: () => Promise<void> | void };
    ...
    findLayerForRn(rn: RN): SourceLayer {
        if (rn.isDepot()) {
            throw new Error(`Cannot use depot RN "${rn.value()}" with layer operations.`);
        }
        ...
        if (fromMod) {
            ...
            if (layer) {
                ...
                throw new Error("Query targets source layer not present in layer pool: " + fromMod)
            }
        } else {
            ...
            throw new Error("Query does not target a source layer while more than one layer is available in pool. Please use the `from` mod to target a layer.")
        }
    ...
    findDepotForRn(rn: RN): Depot {
        ...
        throw new Error(`Depot RN "${rn.value()}" does not specify a depot identifier.`);
    }
}
