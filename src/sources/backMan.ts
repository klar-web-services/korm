...
import { buildBackupPrefixRn, parseBackupTimestamp, type BackupContext, type BackupPayload, type BackupRestoreOptions } from "./backups";
import type { Depot } from "../depot/depot";
import type { SourceLayer } from "./sourceLayer";

export type IntervalUnit = "minute" | "hour" | "day" | "week" | "year" | "decade" | "century" | "millennium";
...
type IntervalStage = "start" | "yearSelected" | "dateSelected" | "timeSelected" | "secondSelected";

type OmitNever<T> = {
    [K in keyof T as T[K] extends never ? never : K]: T[K]
};

type InYearMethod<U extends IntervalUnit, S extends IntervalStage> =
...
type OnDateMethod<U extends IntervalUnit, S extends IntervalStage> =
    U extends "year"
        ? S extends "start"
...
type OnWeekdayMethod<U extends IntervalUnit, S extends IntervalStage> =
    U extends "week"
        ? S extends "start"
...
type AtMethod<U extends IntervalUnit, S extends IntervalStage> =
    U extends "hour"
        ? S extends "start"
...
type OnSecondMethod<U extends IntervalUnit, S extends IntervalStage> =
    U extends "minute"
        ? S extends "start"
...
export type IntervalBuilder<U extends IntervalUnit, S extends IntervalStage> =
    IntervalSpec & OmitNever<{
        /**
...
function assertWeekday(weekday: Weekday): void {
    ...
    if (!WEEKDAY_NAMES.includes(weekday)) {
        throw new Error(`Expected weekday to be one of ${WEEKDAY_NAMES.join(", ")} or 0-6 (0=Monday).`);
    }
}
...
class IntervalBuilderImpl {
    ...
    inYear(year: number): this {
        if (!isMultiYearUnit(this._spec.unit)) {
            throw new Error(`inYear(...) is only valid for decade, century, or millennium units.`);
        }
    ...
    onDate(month: number, day: number): this {
        if (this._spec.unit !== "year" && !isMultiYearUnit(this._spec.unit)) {
            throw new Error(`onDate(...) is only valid for year, decade, century, or millennium units.`);
        }
        if (isMultiYearUnit(this._spec.unit) && this._spec.yearInUnit === undefined) {
            throw new Error(`onDate(...) requires inYear(...) for unit "${this._spec.unit}".`);
        }
    ...
    at(hourOrMinute: number, minute?: number): this {
        ...
        if (this._spec.unit === "hour") {
            throw new Error(`at(hour, minute) is not valid for unit "hour". Use at(minute).`);
        }
        ...
        if (isMultiYearUnit(this._spec.unit)) {
            if (this._spec.yearInUnit === undefined) {
                throw new Error(`at(...) requires inYear(...) for unit "${this._spec.unit}".`);
            }
            if (this._spec.month === undefined || this._spec.day === undefined) {
                throw new Error(`at(...) requires onDate(...) for unit "${this._spec.unit}".`);
            }
        }
    ...
    onSecond(second: number): this {
        ...
        if (this._spec.unit === "day") {
            if (this._spec.hour === undefined || this._spec.minute === undefined) {
                throw new Error(`onSecond(...) requires at(hour, minute) for unit "day".`);
            }
            this._spec.second = second;
            return this;
        }
        if (this._spec.unit === "week") {
            ...
            if (this._spec.hour === undefined || this._spec.minute === undefined) {
                throw new Error(`onSecond(...) requires at(hour, minute) for unit "week".`);
            }
            this._spec.second = second;
            return this;
        }
        if (this._spec.unit === "year") {
            ...
            if (this._spec.hour === undefined || this._spec.minute === undefined) {
                throw new Error(`onSecond(...) requires at(hour, minute) for unit "year".`);
            }
            this._spec.second = second;
            return this;
        }
        if (isMultiYearUnit(this._spec.unit)) {
            if (this._spec.yearInUnit === undefined) {
                throw new Error(`onSecond(...) requires inYear(...) for unit "${this._spec.unit}".`);
            }
            if (this._spec.month === undefined || this._spec.day === undefined) {
                throw new Error(`onSecond(...) requires onDate(...) for unit "${this._spec.unit}".`);
            }
            if (this._spec.hour === undefined || this._spec.minute === undefined) {
                throw new Error(`onSecond(...) requires at(hour, minute) for unit "${this._spec.unit}".`);
            }
            this._spec.second = second;
            return this;
        }
...
function normalizeWeekdayIndex(weekday: Weekday | undefined): number | undefined {
...
function buildDate(year: number, monthIndex: number, day: number, hour: number, minute: number, second: number): Date {
...
function computeNextRun(spec: NormalizedIntervalSpec, from: Date): Date {
    ...
    if (spec.unit === "week") {
        const targetWeekday = normalizeWeekdayIndex(spec.weekday) ?? weekdayIndex(base);
    ...
    if (isMultiYearUnit(spec.unit)) {
        ...
        let candidate = buildDate(targetYear, monthIndex, day, hour, minute, second);
        if (candidate <= base) {
            targetYear += size;
            monthIndex = desiredMonth - 1;
...
class BackupsScheduleStore {
    ...
    private async _createTable(): Promise<void> {
        if (this._layer.type === "sqlite") {
            ...
            if (!db) {
                throw new Error(`Backups require a sqlite layer with a live connection for "${this._layer.identifier}".`);
            }
            db.run(
        ...
        if (this._layer.type === "pg") {
            const db = this._pgDb();
            if (!db) {
                throw new Error(`Backups require a pg layer with a live connection for "${this._layer.identifier}".`);
            }
            await db.unsafe(
                `CREATE TABLE IF NOT EXISTS "${BACKUPS_TABLE}" (
                    "schedule_id" TEXT PRIMARY KEY,
        ...
        if (this._layer.type === "mysql") {
            ...
            if (!pool) {
                throw new Error(`Backups require a mysql layer with a live connection for "${this._layer.identifier}".`);
            }
            await pool.query(
                `CREATE TABLE IF NOT EXISTS \`${BACKUPS_TABLE}\` (
    ...
    async list(): Promise<BackupScheduleRow[]> {
        ...
        if (this._layer.type === "pg") {
            ...
            const rows = await db.unsafe(
                `SELECT schedule_id, next_run_at, last_run_at FROM "${BACKUPS_TABLE}"`
            ) as Array<{ schedule_id: string; next_run_at: unknown; last_run_at?: unknown }>;
            return rows.map((row) => ({
        ...
        if (this._layer.type === "mysql") {
            ...
            const [rows] = await pool.query(
                `SELECT schedule_id, next_run_at, last_run_at FROM \`${BACKUPS_TABLE}\``
            ) as [{ schedule_id: string; next_run_at: unknown; last_run_at?: unknown }[]];
            return rows.map((row) => ({
                scheduleId: row.schedule_id,
                nextRunAt: parseTimestamp(row.next_run_at),
                lastRunAt: parseNullableTimestamp(row.last_run_at),
        ...
        if (this._layer.type === "sqlite") {
            ...
            for (const entry of entries) {
                db.run(
                    ...
                    [entry.scheduleId, entry.layerIdent, entry.intervalJson, entry.nextRunAt, entry.lastRunAt, now, now]
                );
            }
            return;
        }
    ...
    async updateRun(scheduleId: string, nextRunAt: number, lastRunAt: number | null): Promise<void> {
        await this.ensure();
        const now = Date.now();
        if (this._layer.type === "sqlite") {
            const db = this._sqliteDb();
...
export class BackMan {
    ...
    addInterval(layerIdent: string, interval: IntervalSpec): this {
        if (layerIdent !== "*" && this._layers && !this._layers.has(layerIdent)) {
            throw new Error(`Backups layer "${layerIdent}" is not present in the pool.`);
        }
    ...
    /** @internal */
    getPoolConfig(): { intervals: { layerIdent: string; interval: IntervalSpec }[]; retention: { layerIdent: string; retention: BackupRetention }[] } {
        return {
            intervals: this._intervals.map((entry) => ({ layerIdent: entry.layerIdent, interval: entry.interval })),
            retention: Array.from(this._retentionByLayer.entries()).map(([layerIdent, retention]) => ({ layerIdent, retention })),
        };
    }
    ...
    private _buildScheduleSeeds(): BackupScheduleSeed[] {
        ...
        const layerIdents = Array.from(this._layers.keys()).sort((a, b) => a.localeCompare(b));
        const expanded: Array<{ layerIdent: string; interval: NormalizedIntervalSpec }> = [];
        for (const entry of this._intervals) {
            if (entry.layerIdent === "*") {
                for (const layerIdent of layerIdents) {
                    expanded.push({ layerIdent, interval: normalizeIntervalSpec(entry.interval) });
                }
                continue;
            }
            if (!this._layers.has(entry.layerIdent)) {
                throw new Error(`Backups layer "${entry.layerIdent}" is not present in the pool.`);
            }
            expanded.push({ layerIdent: entry.layerIdent, interval: normalizeIntervalSpec(entry.interval) });
        }

        const grouped = new Map<string, { layerIdent: string; interval: NormalizedIntervalSpec; intervalKey: string; count: number }>();
        for (const entry of expanded) {
            const intervalKey = stableIntervalKey(entry.interval);
            const key = `${entry.layerIdent}::${intervalKey}`;
            const group = grouped.get(key) ?? { layerIdent: entry.layerIdent, interval: entry.interval, intervalKey, count: 0 };
            group.count += 1;
        ...
        for (const group of sortedGroups) {
            for (let i = 0; i < group.count; i++) {
                const scheduleId = stableUuidFromSeed(`${group.layerIdent}:${group.intervalKey}:${i}`);
                seeds.push({
                    scheduleId,
    ...
    private async _syncSchedules(): Promise<BackupScheduleEntry[]> {
        ...
        const updates: Array<{ scheduleId: string; nextRunAt: number; lastRunAt: number | null }> = [];
        ...
        if (updates.length > 0) {
            for (const update of updates) {
                await this._store.updateRun(update.scheduleId, update.nextRunAt, update.lastRunAt);
            }
        }
    ...
    private async _handleTimer(state: BackupTimerState): Promise<void> {
        ...
        try {
            ...
            const next = computeNextRun(state.interval, new Date(completedAt)).getTime();
            state.nextRunAt = next;
            state.lastRunAt = completedAt;
            try {
                await this._store?.updateRun(state.scheduleId, state.nextRunAt, completedAt);
            } catch {
                // ignore
            }
            state.running = false;
    ...
    private async _executeBackup(layerIdent: string): Promise<void> {
        ...
        const runner = layer as { backup?: (context: BackupContext) => Promise<void> | void };
    ...
    private _retentionForLayer(layerIdent: string): BackupRetention | undefined {
        return this._retentionByLayer.get(layerIdent) ?? this._retentionByLayer.get("*");
    }
    ...
    private _parseBackupPayload(text: string): BackupPayload {
        ...
        if (payload.version !== 1) {
            throw new Error(`Unsupported backup payload version "${payload.version ?? "unknown"}".`);
        }
        if (typeof payload.layerIdent !== "string" || typeof payload.layerType !== "string") {
    ...
    private async _applyRetention(layerIdent: string): Promise<void> {
        ...
        const entries = files
            .map((file) => ({ rn: file.rn, timestamp: parseBackupTimestamp(file.rn) }))
            .filter((entry): entry is { rn: RN; timestamp: number } => typeof entry.timestamp === "number" && Number.isFinite(entry.timestamp));
    ...
    async play(rn: RN | string, options?: BackupRestoreOptions): Promise<void> {
        ...
        if (!parsed.isDepot() || parsed.pointsTo() !== "depotFile") {
            throw new Error(`Expected backup depot file RN but got "${parsed.value()}".`);
        }
        if (parsed.depotIdent() !== this._depotIdent) {
            throw new Error(`Backup RN depot "${parsed.depotIdent() ?? "unknown"}" does not match "${this._depotIdent}".`);
        }
        ...
        if (!layer) {
            throw new Error(`Backup layer "${payload.layerIdent}" is not present in the pool.`);
        }
        if (layer.type !== payload.layerType) {
            throw new Error(`Backup layer type "${payload.layerType}" does not match "${layer.type}".`);
        }
        const restorer = layer as { restore?: (payload: BackupPayload, options?: BackupRestoreOptions) => Promise<void> | void };
        if (typeof restorer.restore !== "function") {
            throw new Error(`Backup restore is not supported for layer "${payload.layerIdent}".`);
        }
        let release = () => {};
        if (this._locker) {
     ...
     */
    every<U extends IntervalUnit>(unit: U): IntervalBuilder<U, "start"> {
        return new IntervalBuilderImpl(unit) as unknown as IntervalBuilder<U, "start">;
    }
}
