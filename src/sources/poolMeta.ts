...
export type PoolMetaHydratedConfig = {
    metaLayerIdent?: string;
    layers: { ident: string; type: "sqlite" | "pg" | "mysql"; mode: "path" | "url" | "options"; value: unknown }[];
    depots: { ident: string; type: "local" | "s3"; mode: "local" | "s3"; value: unknown }[];
    wal?: PoolMetaWal;
    locks?: PoolMetaLocks;
    backups?: PoolMetaBackups;
};
...
type LayerConfigProvider = SourceLayer & { getPoolConfig?: () => LayerConfigInput };
type DepotConfigProvider = Depot & { getPoolConfig?: () => DepotConfigInput };

type StoredMeta = { config: PoolMetaConfig; createdAt: number; updatedAt: number };
...
function isMetaEnvelope(value: unknown): value is MetaEnvelope {
    ...
    return Boolean(inner && typeof inner === "object" && "type" in (inner as Record<string, unknown>));
}
...
function encodeMetaValue(value: unknown, path: string[] = []): JSONable {
    ...
    if (value instanceof RegExp) {
        return wrapMeta("RegExp", { source: value.source, flags: value.flags } as JSONable);
    }
    if (value instanceof Map) {
        ...
        for (const [key, entry] of value.entries()) {
            ...
            const encodedValue = encodeMetaValue(entry, [...path, `map[${index}].value`]);
    ...
    if (ArrayBuffer.isView(value)) {
        ...
        if (!typeName || !TYPED_ARRAY_NAMES.has(typeName)) {
            throw new Error(`Pool metadata cannot serialize typed array "${typeName ?? "unknown"}" at ${label}.`);
        }
        return wrapMeta(typeName, encodeBinary(value));
    }
    if (Array.isArray(value)) {
        return value.map((entry, index) => encodeMetaValue(entry, [...path, String(index)]));
    }
...
function decodeMetaValue(value: JSONable): unknown {
        ...
        if (type === "Map") {
            ...
            for (const entry of entries) {
                if (!Array.isArray(entry) || entry.length !== 2) {
                    throw new Error("Pool metadata map entry must be a [key, value] tuple.");
                }
    ...
    for (const [key, entry] of Object.entries(value as Record<string, JSONable>)) {
...
async function decryptPayload<T extends JSONable>(payload: PoolEncryptedPayload): Promise<T> {
    return await decrypt(payload as unknown as { value: string; type: "symmetric"; iv: string; authTag: string });
}
...
function normalizeLockMode(mode: LockMode): PoolMetaLocks {
    ...
    const refreshMs = mode.refreshMs ?? Math.max(MIN_LOCK_REFRESH_MS, Math.floor(ttlMs / 2));
    return {
...
function normalizeBackups(backups?: PoolMetaBackups): string {
    ...
    const intervals = [...backups.intervals].sort((a, b) => {
        ...
        return stableStringify(a.interval as JSONable).localeCompare(stableStringify(b.interval as JSONable));
    });
    const retention = [...backups.retention].sort((a, b) => a.layerIdent.localeCompare(b.layerIdent));
    const normalized = {
...
function getDepotConfig(depot: Depot): DepotConfigInput {
    ...
    if (typeof provider.getPoolConfig !== "function") {
        throw new Error("Pool metadata requires depots created via korm.depot.*");
    }
    const config = provider.getPoolConfig();
    if (!config || typeof config !== "object") {
...
async function buildLayerMeta(ident: string, layer: SourceLayer): Promise<PoolMetaLayer> {
    const config = getLayerConfig(layer);
    if (config.type === "sqlite") {
        if (!config.path || typeof config.path !== "string") {
            throw new Error(`Pool metadata requires a sqlite path for layer "${ident}".`);
        }
        return {
            ident,
            type: "sqlite",
    ...
    if (config.type === "pg" || config.type === "mysql") {
        ...
        if (mode === "url") {
            if (typeof config.value !== "string" || !config.value) {
                throw new Error(`Pool metadata requires a connection string for layer "${ident}".`);
            }
        ...
        if (mode === "options") {
            const encoded = encodeMetaValue(config.value, ["layers", ident, "connection"]);
    ...
    throw new Error(`Pool metadata received an unsupported layer config for "${ident}".`);
}

async function buildDepotMeta(ident: string, depot: Depot): Promise<PoolMetaDepot> {
    const config = getDepotConfig(depot);
    if (config.type === "local") {
        if (!config.root || typeof config.root !== "string") {
            throw new Error(`Pool metadata requires a local depot root for "${ident}".`);
        }
        return {
            ident,
            type: "local",
    ...
    if (config.type === "s3") {
        const encoded = encodeMetaValue(config.options, ["depots", ident, "options"]);
        const fingerprint = hashValue(encoded);
        const encrypted = await encryptPayload(encoded);
        return {
            ident,
    ...
    throw new Error(`Pool metadata received an unsupported depot config for "${ident}".`);
}
...
export function comparePoolMetaConfig(current: PoolMetaConfig, stored: PoolMetaConfig): string[] {
    const discrepancies: string[] = [];
    if (stored.version !== current.version) {
        discrepancies.push(`pool version mismatch: expected ${stored.version}, got ${current.version}`);
    }
    if (stored.poolId !== current.poolId) {
        discrepancies.push(`pool id mismatch: expected ${stored.poolId}, got ${current.poolId}`);
    }
    if ((stored.metaLayerIdent ?? null) !== (current.metaLayerIdent ?? null)) {
        discrepancies.push(`meta layer mismatch: expected ${stored.metaLayerIdent ?? "none"}, got ${current.metaLayerIdent ?? "none"}`);
    }

    const storedLayers = new Map(stored.layers.map((layer) => [layer.ident, layer]));
    const currentLayers = new Map(current.layers.map((layer) => [layer.ident, layer]));
    const layerIdents = new Set([...storedLayers.keys(), ...currentLayers.keys()]);
    for (const ident of layerIdents) {
        ...
        if (storedLayer.type !== currentLayer.type) {
            discrepancies.push(`layer type mismatch for ${ident}: expected ${storedLayer.type}, got ${currentLayer.type}`);
            continue;
        }
        if (storedLayer.fingerprint !== currentLayer.fingerprint) {
            if (storedLayer.config.mode === "path" && currentLayer.config.mode === "path") {
                discrepancies.push(
    ...
    const storedDepots = new Map(stored.depots.map((depot) => [depot.ident, depot]));
    const currentDepots = new Map(current.depots.map((depot) => [depot.ident, depot]));
    const depotIdents = new Set([...storedDepots.keys(), ...currentDepots.keys()]);
    for (const ident of depotIdents) {
        ...
        if (storedDepot.type !== currentDepot.type) {
            discrepancies.push(`depot type mismatch for ${ident}: expected ${storedDepot.type}, got ${currentDepot.type}`);
            continue;
        }
        if (storedDepot.fingerprint !== currentDepot.fingerprint) {
            if (storedDepot.config.mode === "local" && currentDepot.config.mode === "local") {
                discrepancies.push(
    ...
    if (!storedWal && currentWal) {
        ...
        if (storedWal.depotIdent !== currentWal.depotIdent) {
            discrepancies.push(`wal depot mismatch: expected ${storedWal.depotIdent}, got ${currentWal.depotIdent}`);
        }
        if (storedWal.walNamespace !== currentWal.walNamespace) {
            discrepancies.push(`wal namespace mismatch: expected ${storedWal.walNamespace}, got ${currentWal.walNamespace}`);
        }
        if (storedWal.retention !== currentWal.retention) {
            discrepancies.push(`wal retention mismatch: expected ${storedWal.retention}, got ${currentWal.retention}`);
        }
        if (storedWal.depotOps !== currentWal.depotOps) {
            discrepancies.push(`wal depotOps mismatch: expected ${storedWal.depotOps}, got ${currentWal.depotOps}`);
        }
    }
    ...
    if (!storedLocks && currentLocks) {
        ...
        if (storedLocks.layerIdent !== currentLocks.layerIdent) {
            discrepancies.push(`lock layer mismatch: expected ${storedLocks.layerIdent}, got ${currentLocks.layerIdent}`);
        }
        if (storedLocks.ttlMs !== currentLocks.ttlMs) {
            discrepancies.push(`lock ttl mismatch: expected ${storedLocks.ttlMs}, got ${currentLocks.ttlMs}`);
        }
        if (storedLocks.retryMs !== currentLocks.retryMs) {
            discrepancies.push(`lock retry mismatch: expected ${storedLocks.retryMs}, got ${currentLocks.retryMs}`);
        }
        if (storedLocks.refreshMs !== currentLocks.refreshMs) {
            discrepancies.push(`lock refresh mismatch: expected ${storedLocks.refreshMs}, got ${currentLocks.refreshMs}`);
        }
    }
...
export async function hydratePoolMetaConfig(config: PoolMetaConfig): Promise<PoolMetaHydratedConfig> {
    ...
    const depots = await Promise.all(config.depots.map(async (depot) => {
...
class LayerPoolMetaStore {
    ...
    private _sqliteDb():
        | { run: (sql: string, params?: unknown[]) => void; prepare: <T>(sql: string) => { get: (arg: string) => T | undefined } }
        | undefined {
        const db = (this._layer as { _db?: unknown })._db;
        if (!db || typeof (db as { run?: unknown }).run !== "function" || typeof (db as { prepare?: unknown }).prepare !== "function") {
            return undefined;
        }
        return db as {
    ...
    private _pgDb(): { unsafe: (sql: string, params?: unknown[]) => Promise<unknown> } | undefined {
        const db = (this._layer as { _db?: unknown })._db;
        if (!db || typeof (db as { unsafe?: unknown }).unsafe !== "function") {
            return undefined;
        }
        return db as { unsafe: (sql: string, params?: unknown[]) => Promise<unknown> };
    }

    private _mysqlPool(): { query: (sql: string, params?: unknown[]) => Promise<unknown> } | undefined {
        const pool = (this._layer as { _pool?: unknown })._pool;
        if (!pool || typeof (pool as { query?: unknown }).query !== "function") {
            return undefined;
        }
        return pool as { query: (sql: string, params?: unknown[]) => Promise<unknown> };
    }
    ...
    private async _createTable(): Promise<void> {
        if (this._layer.type === "sqlite") {
            ...
            if (!db) {
                throw new Error(`Pool metadata requires a sqlite layer with a live connection for "${this._layer.identifier}".`);
            }
            db.run(
        ...
        if (this._layer.type === "pg") {
            const db = this._pgDb();
            if (!db) {
                throw new Error(`Pool metadata requires a pg layer with a live connection for "${this._layer.identifier}".`);
            }
            await db.unsafe(
                `CREATE TABLE IF NOT EXISTS "${POOL_META_TABLE}" (
                    "id" TEXT PRIMARY KEY,
        ...
        if (this._layer.type === "mysql") {
            ...
            if (!pool) {
                throw new Error(`Pool metadata requires a mysql layer with a live connection for "${this._layer.identifier}".`);
            }
            await pool.query(
                `CREATE TABLE IF NOT EXISTS \`${POOL_META_TABLE}\` (
        ...
        throw new Error(`Pool metadata does not support layer type "${this._layer.type}".`);
    }
    ...
    async read(): Promise<StoredMeta | undefined> {
        ...
        if (this._layer.type === "sqlite") {
            ...
            const row = db.prepare<{ config: string; created_at: number; updated_at: number }>(
            ...
            return { config: parsed, createdAt: row.created_at, updatedAt: row.updated_at };
        }
        if (this._layer.type === "pg") {
            const db = this._pgDb();
            if (!db) return undefined;
            ...
            return { config: config as PoolMetaConfig, createdAt: row.created_at, updatedAt: row.updated_at };
        }
        if (this._layer.type === "mysql") {
            const pool = this._mysqlPool();
            ...
            if (!row?.config) return undefined;
            const config = typeof row.config === "string" ? JSON.parse(row.config) : row.config;
            return { config: config as PoolMetaConfig, createdAt: row.created_at ?? 0, updatedAt: row.updated_at ?? 0 };
        }
        return undefined;
    }
...
export async function readPoolMetaConfig(layer: SourceLayer): Promise<PoolMetaConfig | undefined> {
    const store = new LayerPoolMetaStore(layer);
    const row = await store.read();
    if (!row) return undefined;
    if (row.config.version !== POOL_META_VERSION) {
        throw new Error(`Pool metadata version ${row.config.version} is not supported.`);
    }
    return row.config;
}

export async function writePoolMetaConfig(layer: SourceLayer, config: PoolMetaConfig): Promise<void> {
    const store = new LayerPoolMetaStore(layer);
...
export async function findExistingPoolMeta(layers: Map<string, SourceLayer>): Promise<Array<{ ident: string; config: PoolMetaConfig }>> {
    ...
    return entries.filter(Boolean) as Array<{ ident: string; config: PoolMetaConfig }>;
}

export function formatPoolMetaMismatch(params: { discrepancies: string[]; layerIdents: string[] }): Error {
