...
import { buildBackupPrefixRn, buildBackupRn, parseBackupTimestamp } from "./backups";
...
describe("BackMan interval builder", () => {
    ...
    test("validates required steps for day/hour/year units", () => {
        expect(() => {
            (new BackMan().every("day") as unknown as { onSecond: (value: number) => void }).onSecond(5);
        }).toThrow("at(hour, minute)");
        expect(() => {
            (new BackMan().every("hour") as unknown as { onSecond: (value: number) => void }).onSecond(10);
        }).toThrow("at(minute)");
        expect(() => {
            (new BackMan().every("year") as unknown as { at: (hour: number, minute: number) => void }).at(1, 2);
        }).toThrow("onDate");
    });

    test("validates multi-year scheduling requirements", () => {
        expect(() => {
            (new BackMan().every("decade") as unknown as { onDate: (month: number, day: number) => void }).onDate(1, 1);
        }).toThrow("inYear");
        expect(() => {
            (new BackMan().every("century") as unknown as { at: (hour: number, minute: number) => void }).at(0, 0);
        }).toThrow("inYear");
        const spec = new BackMan().every("millennium").inYear(2).onDate(1, 1).at(0, 0);
    ...
    test("registers intervals and retention for known layers", () => {
        const manager = new BackMan().conveyLayers(new Map([["main", {} as never]]));
    ...
    test("applies retention policy to backup files", async () => {
        ...
        const depot = korm.depot.local(depotRoot);
        ...
        try {
            ...
            for (const stamp of stamps) {
                ...
                const file = new FloatingDepotFile(rn, new Blob(["{}"], { type: "application/json" }));
                await depot.createFile(file);
            }

            await (manager as any)._applyRetention("main");
            const remaining = await depot.listFiles(buildBackupPrefixRn("backups", "main"));
    ...
    test("play restores backups into a fresh layer", async () => {
        ...
        const pool = korm.pool()
            .setLayers({ layer: sourceLayer, ident: "main" })
            .setDepots({ depot, ident: "backups" })
            .withMeta({ layerIdent: "main" })
            .open();
        ...
        try {
            ...
            const created = await korm.item<{ name: string }>(pool).from.data({
                namespace: "users",
                kind: "basic",
                data: { name: "Ada" }
            }).create();
            ...
            const backups = await depot.listFiles(buildBackupPrefixRn("backups", "main"));
            ...
            const rows = restoreLayer._db.prepare(`SELECT * FROM "__items__users__basic"`).all() as Array<{ name?: string }>;
            expect(rows.length).toBe(1);
            expect(rows[0]?.name).toBe("Ada");

            const poolRows = restoreLayer._db.prepare(`SELECT id FROM "__korm_pool__"`).all() as Array<{ id?: string }>;
