...
export class PgLayer implements SourceLayer {
    ...
    async backup(context: BackupContext): Promise<void> {
        ...
        for (const row of tables) {
            ...
            const rows = await this._db.unsafe(`SELECT * FROM ${safeName}`) as Record<string, unknown>[];
    ...
    async restore(payload: BackupPayload, options?: BackupRestoreOptions): Promise<void> {
        if (payload.layerType !== "pg") {
            throw new Error(`Backup payload layer type "${payload.layerType}" does not match pg.`);
        }
        ...
        for (const table of payload.tables) {
            ...
            if (rawName === poolTable) {
                ...
                for (const key of columns) {
                    ...
                    if (rawName.startsWith("__items__") && key === "rnId") {
                        ...
                        columnDefs.push(`${columnName} ${this._inferBackupPgType(rows, key)}`);
                    }
                }
                if (columnDefs.length > 0) {
                    await this._unsafe(`CREATE TABLE IF NOT EXISTS ${safeName} (${columnDefs.join(", ")})`);
                    schemaChanged = true;
                }
            }
            ...
            for (const key of missing) {
                const columnName = this._quoteIdent(key);
                if (rawName.startsWith("__items__") && key === "rnId") {
                    await this._unsafe(`ALTER TABLE ${safeName} ADD COLUMN ${columnName} TEXT`);
                } else {
                    await this._unsafe(`ALTER TABLE ${safeName} ADD COLUMN ${columnName} ${this._inferBackupPgType(rows, key)}`);
                }
                schemaChanged = true;
            }
            ...
            for (const row of rows) {
                const values = columns.map((key) => this._encodePgValue((row as any)[key]));
    ...
    private _deriveIdentifier(connectionStringOrOptions: string | SQL.Options): string {
    ...
    private async _ensureDomains(): Promise<void> {
        ...
        try {
            ...
            const existingSet = new Set((existing || []).map((row) => String(row.typname)));
    ...
    private _inferBackupPgType(rows: Record<string, unknown>[], key: string): string {
        for (const row of rows) {
    ...
    private async _decodeRowUsingTableInfo<T extends Record<string, any>>(
        ...
        for (const [k, v] of Object.entries(out)) {
            ...
            const isEncryptedDomain =
                info.domainName === PG_ENCRYPTED_DOMAIN || info.udtName === PG_ENCRYPTED_DOMAIN;
            const isJson = info.dataType === "JSONB" || info.dataType === "JSON" || isEncryptedDomain;
    ...
    async updateItem<T extends JSONable>(item: UncommittedItem<T>, options?: PersistOptions): Promise<DbChangeResult<T>> {
        ...
        const currentRow = (await this._unsafe(
    ...
    async deleteItem(rn: RN): Promise<DbDeleteResult> {
        ...
        const safeTableName = this._quoteIdent(rawTableName);
        try {
            await this._unsafe(`DELETE FROM ${safeTableName} WHERE "rnId" = $1`, [rn.id!]);
            return { success: true };
        } catch (error) {
    ...
    private _buildQueryString(
        ...
        const buildJsonComparison = (jsonText: string, operator: string, value: any): string => {
            ...
            if (node.type === "comparison") {
                ...
                if (node.property.includes(".")) {
                    ...
                    if (node.operator === "IN") {
                        ...
                        if (valueType === "number") {
                            return buildInClause(`(${lhs})::double precision`, values, pushParam, hasNull);
                        }
                        if (valueType === "boolean") {
                            return buildInClause(`(${lhs})::boolean`, values, pushParam, hasNull);
                        }
    ...
    async ensureTables(
        ...
        if (!exists) {
            ...
            for (const key in item.data) {
                ...
                else if (t === "DOUBLE PRECISION") createString += `${columnName} DOUBLE PRECISION, `;
                else if (t === "BOOLEAN") createString += `${columnName} BOOLEAN, `;
                else if (t === "JSONB") createString += `${columnName} JSONB, `;
                else if (t === PG_RN_DOMAIN) createString += `${columnName} ${PG_RN_DOMAIN}, `;
                else if (t === PG_ENCRYPTED_DOMAIN) createString += `${columnName} ${PG_ENCRYPTED_DOMAIN}, `;
        ...
        for (const key in item.data) {
            ...
            const existingDomain = (existingColumn?.domainName || existingColumn?.udtName || "").toLowerCase();

            const normalizeType = (type: string, domainName?: string): string => {
                const upper = (type || "").toUpperCase();
                const domain = (domainName || "").toLowerCase();
                if (domain === PG_RN_DOMAIN || upper.toLowerCase() === PG_RN_DOMAIN) return "TEXT";
                if (domain === PG_ENCRYPTED_DOMAIN || upper.toLowerCase() === PG_ENCRYPTED_DOMAIN) return "JSONB";
                if (upper.includes("CHAR") || upper.includes("TEXT")) return "TEXT";
                if (upper === "INTEGER") return "INTEGER";
                if (upper.includes("DOUBLE") || upper.includes("REAL")) return "DOUBLE PRECISION";
                if (upper.includes("NUMERIC") || upper.includes("DECIMAL")) return "DOUBLE PRECISION";
            ...
            let normalizedExisting = normalizeType(existingColumnType, existingDomain);
            let normalizedWanted = normalizeType(columnWantedType);

            if (normalizedWanted === "INTEGER" && normalizedExisting === "DOUBLE PRECISION") {
                normalizedWanted = "DOUBLE PRECISION";
            }
            if (normalizedExisting === "JSONB" && normalizedWanted === "TEXT" && typeof v === "string") {
                try {
                    JSON.parse(v);
            ...
            if (!existingColumnType) {
                ...
                await this._unsafe(`ALTER TABLE ${tableName} DROP COLUMN ${columnName}`);
                await this._unsafe(`ALTER TABLE ${tableName} ADD COLUMN ${columnName} ${columnWantedType}`);
                await this._clearPlanCache();
                schemaChanged = true;
