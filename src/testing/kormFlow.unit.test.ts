...
async function createTempPool() {
    ...
    const pool = korm.pool()
        .setLayers({ layer: korm.layers.sqlite(dbPath), ident: "main" })
        .withMeta({ layerIdent: "main" })
        .withLocks({ layerIdent: "main" })
        .setDepots(
            { depot: korm.depot.local(depotRoot), ident: "files" },
            { depot: korm.depot.local(walRoot), ident: "wal" }
        )
        .withWal({
            depotIdent: "wal",
            walNamespace: "unit",
...
describe("korm core flow", () => {
    test("creates, queries, resolves, and runs WAL/backups", async () => {
        ...
        try {
            ...
            const created = (await korm.item<User>(pool).from.data({
                namespace: "users",
                kind: "basic",
                data: {
                    name: "Ada",
            ...
            const updated = (await created.update({ name: "Ada2" }).commit()).unwrap();

            const groupResult = await korm.item<Group>(pool).from.data({
                namespace: "groups",
                kind: "team",
            ...
            const floating = korm.item<User>(pool).from.data({
                namespace: "users",
                kind: "basic",
    ...
    test("discovers a pool from metadata and can reset meta", async () => {
        ...
        const pool = korm.pool()
            .setLayers({ layer: korm.layers.sqlite(dbPath), ident: "main" })
            .withMeta({ layerIdent: "main" })
            .setDepots({ depot: korm.depot.local(depotRoot), ident: "files" })
            .open();
