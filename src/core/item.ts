...
function resolvedDataFor<T extends JSONable>(target: object, fallback?: T): T | undefined {
...
function isRnValue(value: unknown): value is RN<JSONable> | string {
    ...
    if (typeof value === "object" && "__RN__" in (value as Record<string, unknown>)) return true;
    return false;
}
...
function normalizeRefOverride(value: unknown): { rawValue: unknown; resolvedValue: unknown } {
    if (isDepotFile(value)) {
        return { rawValue: value.rn.value(), resolvedValue: value };
    }
    if (value && typeof value === "object" && "__RN__" in (value as Record<string, unknown>)) {
...
export class Item<T extends JSONable> {
    ...
    constructor(layerPool: LayerPool, data?: T, rn?: RN<T>) {
        this._data = data;
        this._rn = rn;
    ...
    /** True if the item has no RN or data. */
    isEmpty(): boolean {
        const predicates = [
            !this.data || !this.rn,
            this.data === ({} as T),
    ...
    update(delta: DeepPartial<T>): Result<UncommittedItem<T>> {
        try {
            ...
            const nextEncryptionMeta = encryptionMeta ? cloneEncryptionMeta(encryptionMeta) : undefined;
            const meta = getResolvedMeta(this);
            if (!meta) {
                const newData = deepMerge(this._data as T, delta);
                const uncommitted = new UncommittedItem<T>(this._layerPool, this._rn, newData);
            ...
            const overrides: Array<{ path: PathKey[]; rawValue: unknown; resolvedValue: unknown }> = [];
            ...
            const isOverriddenByParent = (path: PathKey[]): boolean => {
                for (let i = path.length - 1; i > 0; i--) {
                    const parentValue = getValueAtPath(delta as unknown, path.slice(0, i));
            ...
            for (const ref of meta.refs) {
                ...
                if (isRefOverride(deltaValue)) {
                    ...
                    overrides.push({ path: ref.path, rawValue: normalized.rawValue, resolvedValue: normalized.resolvedValue });
                    continue;
                }
            ...
            for (const override of overrides) {
                ...
                setValueAtPath(nextResolved as unknown, override.path, override.resolvedValue);
            }
            nextMeta.rawData = nextRaw;
            const uncommitted = new UncommittedItem<T>(this._layerPool, this._rn, nextRaw as T);
 ...
 * Use the `.from` helpers to move into the create/query/fetch flows.
 */
export class UninitializedItem<T extends JSONable> {
    ...
    public readonly from = {
        ...
        rn: async <const Paths extends readonly string[] = []>(
            ...
            try {
                if (rn.pointsTo() !== "item") {
                    throw new Error(`Trying to fetch a collection RN "${rn.value()}" with from.rn`);
                }
                if (!rn.namespace || !rn.kind || !rn.id) {
                    throw new Error(`Invalid RN "${rn.value()}"`);
                }
                const collectionRes = RN.create<T>(rn.namespace, rn.kind, "*", new Map(rn.mods ?? []));
                ...
                if (items.length !== 1) {
                    return new Result({
                        success: false,
                        error: new Error(`Expected 1 item for RN "${rn.value()}" but got ${items.length}.`),
                    });
                }
                return new Result({ success: true, data: items[0] as Item<ResolvePaths<T, Paths>> });
            } catch (error) {
                return new Result({
                    success: false,
        ...
        query: (rn: RN<T>): QueryBuilder<T> => {
            if (rn.pointsTo() !== "collection") {
                throw new Error(`Trying to run query on item-specific RN "${rn.value()}"`)
            }
        ...
        data: (params: FromDataParams<T>): Result<FloatingItem<T>> => {
            const rnRes = RN.create<T>(params.namespace, params.kind, crypto.randomUUID());
            if (rnRes.isErr()) {
...
export class FloatingItem<T extends JSONable> {
    ...
    private async _createLocked(): Promise<Result<Item<T>>> {
        ...
        const prepared = await prepareWriteData(this._data, encryptionMeta, this._layerPool, {
            depotOps,
            skipDepotUpload: captureDepotOps,
        });
        if (prepared.data !== undefined) {
            persistItem = new FloatingItem<T>(this._layerPool, this._rn, prepared.data as T);
            nextEncryptionMeta = prepared.meta;
            if (nextEncryptionMeta) {
                setEncryptionMeta(persistItem, nextEncryptionMeta);
            }
        }
        let walHandle: Awaited<ReturnType<NonNullable<typeof wal>["stage"]>> | undefined;
        try {
            ...
            return new Result({ success: false, error: new Error("Failed to create item " + this._rn?.value()) });
        } catch (error) {
            if (wal && walHandle) {
                if (captureDepotOps) {
                    try {
...
export class UncommittedItem<T extends JSONable> {
    ...
    private async _commitSimpleLocked(): Promise<Result<Item<T>>> {
        ...
        if (prepared.data !== undefined) {
            persistItem = new UncommittedItem<T>(this._layerPool, this._rn, prepared.data as T);
            nextEncryptionMeta = prepared.meta;
            if (nextEncryptionMeta) {
                setEncryptionMeta(persistItem, nextEncryptionMeta);
            }
        ...
        try {
            ...
            return new Result({ success: false, error: new Error("Failed to commit item " + this._rn?.value()) });
        } catch (error) {
            if (wal && walHandle) {
                if (captureDepotOps) {
                    try {
    ...
    private async _commitWithResolvedLocked(meta: ResolvedMeta): Promise<Result<Item<T>>> {
        const wal = this._layerPool.wal;
        const captureDepotOps = wal?.depotOpsEnabled ?? false;

        const depotOps: DepotFileBase[] | undefined = captureDepotOps ? [] : undefined;
        const updatesByRn = new Map<string, {
            meta: ResolvedMeta;
            nextRaw: JSONable;
        ...
        const collectUpdates = async (currentMeta: ResolvedMeta) => {
            ...
            for (const ref of currentMeta.refs) {
                ...
                if (childEncryptionMeta) {
                    const applied = await applyEncryptionMeta(childRaw as JSONable, childEncryptionMeta);
                    ...
                    updatesByRn.set(childMeta.rn.value(), { meta: childMeta, nextRaw: childRaw });
                }
        ...
        for (const update of updatesByRn.values()) {
            const prepared = await prepareWriteData(update.nextRaw as JSONable, update.encryptionMeta, this._layerPool, {
                depotOps,
                skipDepotUpload: captureDepotOps,
            });
            if (prepared.data === undefined) {
            ...
            const item = new UncommittedItem<JSONable>(this._layerPool, update.meta.rn, prepared.data);
        ...
        const rootPrepared = await prepareWriteData(rootRaw as JSONable, rootEncryptionMeta, this._layerPool, {
            depotOps,
            skipDepotUpload: captureDepotOps,
        });
        ...
        const rootItem = new UncommittedItem<T>(this._layerPool, this._rn, rootPersistRaw as T);
        ...
        try {
            ...
            for (const pending of pendingUpdates) {
                ...
                if (!result.success) {
                    ...
                    return new Result({ success: false, error: result.error ?? new Error("Failed to update resolved references.") });
                }
                applied.push(result);
            }
            ...
            if (!rootResult.success) {
                ...
                return new Result({ success: false, error: rootResult.error ?? new Error("Failed to commit item " + this._rn?.value()) });
            }
            if (wal && walHandle) {
                await wal.markDone(walHandle);
            }
