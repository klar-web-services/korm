...
import { cloneJson, deepEqualJson, setResolvedMeta, type PathKey, type ResolvedMeta } from "./resolveMeta";
...
type SplitBracketed<S extends string, Acc extends string[] = []> =
...
type SplitPath<P extends string, Acc extends string[] = []> =
 ...
 */
export type ResolvePaths<T, Paths extends readonly string[]> = IsTuple<Paths> extends true
    ? ResolvePathsTuple<T, Paths>
    : T;

type ResolvePathsTuple<T, Paths extends readonly string[]> = Paths extends readonly [
...
type ResolveAt<T, Segs extends readonly string[]> = Segs extends readonly []
    ...
    : T extends RN<infer U>
      ? ResolveAt<U, Segs>
      : Segs extends readonly [infer Head extends string, ...infer Tail extends readonly string[]]
        ? ResolveSegment<T, Head, Tail>
        : T;

type ResolveSegment<T, Head extends string, Tail extends readonly string[]> =
    Head extends "*"
...
type ResolveProp<T, Key extends string, Tail extends readonly string[]> =
...
type ResolveWildcard<T, Tail extends readonly string[]> =
    T extends readonly unknown[]
...
type ResolveIndex<T, Index extends string, Tail extends readonly string[]> =
    T extends readonly unknown[]
        ? Index extends "*"
            ? ResolveArrayAll<T, Tail>
...
type ResolveArrayAll<T extends readonly unknown[], Tail extends readonly string[]> =
    IsTuple<T> extends true
        ? { [I in keyof T]: ResolveAt<T[I], Tail> }
        : ResolveAt<T[number], Tail>[];

type ResolveArrayIndex<T extends readonly unknown[], Index extends number, Tail extends readonly string[]> =
    IsTuple<T> extends true
...
export class QueryBuilder<T extends JSONable> {
     ...
     * Use `resolvePaths` to materialize RN references in the returned data.
     */
    async get<const Paths extends readonly string[] = []>(
        ...
        if (root && this.rn.namespace && this.rn.kind) {
            try {
                columnKinds = await layer.getColumnKinds(this.rn.namespace, this.rn.kind);
            } catch {
        ...
        const autoResolvePaths =
            resolvePaths.length === 0 && root
                ...
                : [];
        const shouldAutoResolve = resolvePaths.length === 0 && autoResolvePaths.length > 0;

        let queryForDb: QueryBuilder<T> = this;
        let needsInMemoryFilter = false;
        if (root) {
            const resolveInfo = parseResolvePaths(shouldAutoResolve ? autoResolvePaths : resolvePaths);
            const dbResult = buildDbComponent(root, resolveInfo.firstSegments, columnKinds);
            needsInMemoryFilter = dbResult.didFilter;
            if (dbResult.component) {
                queryForDb = new QueryBuilder<T>(this._rn, this._futureItem).where(dbResult.component);
            } else {
        ...
        if (resolvePaths.length > 0) {
            const resolved = await resolveItems(baseItems, resolvePaths, pool, { allowMissing });
            if (resolved.isErr()) {
        ...
        if (shouldAutoResolve) {
            ...
            const resolved = await resolveItems(clones, autoResolvePaths, pool, { allowMissing: true, projection: "auto" });
            if (resolved.isErr()) {
                return resolved as Result<Item<ResolvePaths<T, Paths>>[]>;
            }
            const filtered = filterItemsByQuery(
            ...
            const output = baseItems.filter((item) => item.rn && matched.has(item.rn.value()));
            return new Result({ success: true, data: output as Item<ResolvePaths<T, Paths>>[] });
        }

        const filtered = filterItemsByQuery(baseItems as Item<ResolvePaths<T, Paths>>[], root);
        return new Result({ success: true, data: filtered });
    }
...
const tsIdentifierUnicodeRegex = /^[\p{ID_Start}_$][\p{ID_Continue}\u200C\u200D$_]*$/u;
...
function isValidTsIdentifier(name: string): boolean {
    ...
    if (!hasNonAscii) {
        ...
        const isStart = first === 36 || first === 95 || (first >= 65 && first <= 90) || (first >= 97 && first <= 122);
        if (!isStart) return false;
        for (let i = 1; i < name.length; i++) {
            const code = name.charCodeAt(i);
            const isContinue = code === 36 || code === 95 || (code >= 65 && code <= 90) || (code >= 97 && code <= 122) || (code >= 48 && code <= 57);
...
type PendingRn = {
    rn: RN<JSONable>;
    targets: { set: Setter; remaining: PathSegment[]; path: PathKey[]; meta: ResolvedMeta }[];
};
...
function unwrapEncrypted(value: unknown): unknown {
    ...
    if (maybeEncrypt.__ENCRYPT__ === true && typeof maybeEncrypt.reveal === "function") {
    ...
    while (i < trimmed.length) {
        ...
        if (ch === "[") {
            ...
            if (content === "*") {
                ...
                throw new Error(`Invalid resolve path "${path}": invalid index "${content}".`);
            }
            i = end + 1;
            if (i < trimmed.length && trimmed[i] !== "." && trimmed[i] !== "[") {
                throw new Error(`Invalid resolve path "${path}": missing "." before "${trimmed[i]}".`);
            }
            continue;
        }
    ...
    while (i < trimmed.length) {
        ...
        if (ch === "[") {
            ...
            if (content === "*") {
                ...
                throw new Error(`Invalid query path "${path}": invalid index "${content}".`);
            }
            i = end + 1;
            if (i < trimmed.length && trimmed[i] !== "." && trimmed[i] !== "[") {
                throw new Error(`Invalid query path "${path}": missing "." before "${trimmed[i]}".`);
            }
            continue;
        }
...
function parseResolvePaths(paths: readonly string[]): { parsed: PathSegment[][]; firstSegments: PathSegment[] } {
...
function collectResolvePathsFromQuery(root: _QueryComponent, columnKinds: Map<string, ColumnKind>): string[] {
    const paths: string[] = [];
    const seen = new Set<string>();
...
function expandSegment(value: unknown, seg: PathSegment): { value: unknown; set: Setter; pathKey: PathKey }[] {
    if (seg.kind === "prop") {
...
function segmentsMatch(resolveSeg: PathSegment, propSeg: PathSegment): boolean {
    if (resolveSeg.kind === "wildcard" || resolveSeg.kind === "indexWildcard") return true;
    if (propSeg.kind === "wildcard" || propSeg.kind === "indexWildcard") return true;
...
function buildDbComponent(
    ...
    if (node.method === "NOT") {
        ...
        const childResult = buildDbComponent(child, resolveFirstSegments, columnKinds);
        if (!childResult.component) return { component: null, didFilter: true };
        return {
            component: { type: "group", method: "NOT", components: [childResult.component] },
    ...
    const didFilter = childResults.some((result) => result.didFilter) || children.length !== node.components.length;
    ...
    return { component: { type: "group", method: node.method, components: children }, didFilter };
}

function coerceRn(value: unknown, cache: Map<string, RN<JSONable>>): RN<JSONable> | undefined {
...
function evaluateComponent(
    ...
    if (node.method === "AND") {
        return node.components.every((child) => evaluateComponent(child, data, pathCache, likeCache));
    }

    return node.components.some((child) => evaluateComponent(child, data, pathCache, likeCache));
}
...
function collectValuesAtPath(value: unknown, segments: PathSegment[]): unknown[] {
...
function normalizeComparable(value: unknown): unknown {
    ...
    if ("__RN__" in (value as Record<string, unknown>) && typeof (value as { value?: () => string }).value === "function") {
...
async function resolveItems<T extends JSONable, Paths extends readonly string[]>(
    items: Item<T>[],
    paths: Paths,
    pool: LayerPool,
    ...
    try {
        ...
        if (parsedPaths.length === 0 || items.length === 0) {
            return new Result({ success: true, data: items as Item<ResolvePaths<T, Paths>>[] });
        }
        ...
        for (const item of items) {
            ...
            for (const segments of parsedPaths) {
                pending.push({ value: resolvedData, set: noop, remaining: segments, path: [], meta });
            }
        }
        ...
        const createResolvedNode = (
            ...
            const meta: ResolvedMeta = { rn, rawData: rawClone, resolvedData: resolvedClone, refs: [] };
            if (resolvedClone !== null && typeof resolvedClone === "object") {
                setResolvedMeta(resolvedClone as object, meta);
                if (encryptionMeta) {
                    setEncryptionMeta(resolvedClone as object, cloneEncryptionMeta(encryptionMeta));
                }
            }
        ...
        while (pending.length > 0) {
            ...
            const enqueue = (rn: RN<JSONable>, set: Setter, remaining: PathSegment[], path: PathKey[], meta: ResolvedMeta) => {
                const key = rn.value();
                const existing = pendingRns.get(key);
                if (existing) {
                    existing.targets.push({ set, remaining, path, meta });
                } else {
                    pendingRns.set(key, { rn, targets: [{ set, remaining, path, meta }] });
                }
            };
            ...
            for (const task of pending) {
                ...
                for (const child of children) {
                    ...
                    if (rn) {
                        const cached = resolvedCache.get(rn.value());
                        if (cached !== undefined) {
                            const { value: resolvedValue, meta: childMeta } = createResolvedNode(
                                rn,
                                cached.data,
                                cached.encryptionMeta
                            );
                            ...
                            if (!rn.isDepot() && tail.length > 0 && typeof resolvedValue === "object" && resolvedValue !== null) {
                                next.push({ value: resolvedValue, set: noop, remaining: tail, path: [], meta: childMeta });
                            }
                        } else {
                            enqueue(rn, child.set, tail, childPath, task.meta);
                        }
                    } else if (tail.length > 0) {
                        next.push({ value: child.value, set: child.set, remaining: tail, path: childPath, meta: task.meta });
                    }
                }
            }

            if (pendingRns.size > 0) {
                const resolvedResult = await fetchResolvedData(pool, pendingRns, { projectionMode });
                ...
                for (const pendingEntry of pendingRns.values()) {
                    ...
                    for (const target of pendingEntry.targets) {
                        ...
                        if (!pendingEntry.rn.isDepot() && target.remaining.length > 0 && typeof resolvedValue === "object" && resolvedValue !== null) {
                            next.push({
                                value: resolvedValue,
                                set: noop,
...
async function fetchResolvedData(
    ...
    try {
        ...
        const projectionFromTargets = (targets: PendingRn["targets"]): { full: boolean; columns: Set<string> } => {
        ...
        for (const entry of pendingRns.values()) {
            ...
            const projectionInfo =
                projectionMode === "auto" ? projectionFromTargets(entry.targets) : { full: true, columns: new Set<string>() };
        ...
        for (const group of groups.values()) {
            ...
            const rnRes = RN.create(group.namespace, group.kind, "*", new Map(group.mods));
            if (rnRes.isErr()) {
            ...
            if (projectionMode === "auto" && projection) {
                try {
                    const columnKinds = await group.layer.getColumnKinds(group.namespace, group.kind);
            ...
            for (const chunk of chunks) {
                ...
                const query = new QueryBuilder<JSONable>(collectionRn, new UninitializedItem<JSONable>(pool));
                if (projection) {
                ...
                for (const item of result.unwrap()) {
                    ...
                    if (item.data !== undefined) {
                        ...
                        resolved.set(key, {
                            ...
                            encryptionMeta: encryptionMeta ? cloneEncryptionMeta(encryptionMeta) : undefined,
                        });
                    }
                }
            }
