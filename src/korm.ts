...
import { BackMan, type IntervalFactory, type IntervalSpec } from "./sources/backMan";
import { eq, and, gt, gte, inList, like, lt, lte, not, or } from "./core/queryFns";
import { tx } from "./core/txBuilder";
import { Encrypt } from "./security/encryption";
import type { WalMode } from "./wal/wal";
import type { LockMode } from "./sources/lockStore";
import { dropPoolMetaConfig, hydratePoolMetaConfig, readPoolMetaConfig, type PoolMetaBackups, type PoolMetaMode } from "./sources/poolMeta";
...
type LayerInput = SourceLayer | { layer: SourceLayer; ident: string };
type DepotInput = Depot | { depot: Depot; ident: string };
...
type LayerIdentFrom<Entries extends readonly unknown[]> = LayerIdentOf<Entries[number]>;
type DepotIdentFrom<Entries extends readonly unknown[]> = DepotIdentOf<Entries[number]>;
type LayerIdentOrWildcard<LIds extends string> = LIds | "*";
type RetainCallable<LIds extends string, DIds extends string> = BackupsBuilder<LIds, DIds> & (() => BackupsBuilder<LIds, DIds>);
...
type WalModeFor<D extends string> = Omit<WalMode, "depotIdent"> & { depotIdent: D };
type LockModeFor<L extends string> = Omit<LockMode, "layerIdent"> & { layerIdent: L };
type PoolMetaModeFor<L extends string> = Omit<PoolMetaMode, "layerIdent"> & { layerIdent: L };
...
type PoolBuilderStart = {
     ...
     */
    setLayers<const Entries extends readonly LayerInput[]>(...entries: Entries): PoolBuilderWithLayers<LayerIdentFrom<Entries>>;
};
...
type PoolBuilderWithLayers<LIds extends string> = {
     ...
     */
    setDepots<const Entries extends readonly DepotInput[]>(...entries: Entries): PoolBuilderWithDepots<LIds, DepotIdentFrom<Entries>>;
    /**
     ...
     */
    withLocks: (opts: LockModeFor<LIds>) => PoolBuilderWithLayers<LIds>;
...
type PoolBuilderWithDepots<LIds extends string, DIds extends string> = {
     ...
     * Next: call `withWal(...)`, `backups(...)`, or stop.
     */
    setDepots<const Entries extends readonly DepotInput[]>(...entries: Entries): PoolBuilderWithDepots<LIds, DIds | DepotIdentFrom<Entries>>;
    /**
     * Enable pool metadata tracking using a layer in this pool.
     ...
     * Next: call `withWal(...)`, `backups(...)`, or stop.
     */
    withLocks: (opts: LockModeFor<LIds>) => PoolBuilderWithDepots<LIds, DIds>;
    /**
...
type BackupsBuilder<LIds extends string, DIds extends string> = PoolBuilderWithDepots<LIds, DIds> & {
    /**
     * Register a backup interval for a specific layer ident.
     * Next: call `addInterval(...)` again or stop.
     */
    addInterval: (layerIdent: LayerIdentOrWildcard<LIds>, interval: IntervalSpec) => BackupsBuilder<LIds, DIds>;
    /**
...
function item<T extends JSONable>(sourcePool: "Did you forget to call .open()?"): UninitializedItem<T>;
function item<T extends JSONable>(sourcePool: LayerPool | "Did you forget to call .open()?"): UninitializedItem<T> {
    return new UninitializedItem<T>(sourcePool as LayerPool);
}

function buildBackupsManager(config: PoolMetaBackups, layers: Map<string, SourceLayer>, depots: Map<string, Depot>): { depotIdent: string; manager: BackMan } {
    const depot = depots.get(config.depotIdent);
    if (!depot) {
        throw new Error(`Backups depot "${config.depotIdent}" is not present in the discovered pool.`);
    }
...
async function discover(sourceLayer: SourceLayer): Promise<LayerPool> {
    ...
    if (!meta) {
        throw new Error(`Pool metadata not found in layer "${sourceLayer.identifier}".`);
    }
    ...
    for (const layer of hydrated.layers) {
        if (layer.type === "sqlite") {
            if (layer.mode !== "path" || typeof layer.value !== "string") {
                throw new Error(`Pool metadata has invalid sqlite config for "${layer.ident}".`);
            }
        ...
        if (layer.type === "pg") {
            if (layer.mode === "url") {
                if (typeof layer.value !== "string") {
                    throw new Error(`Pool metadata has invalid pg connection string for "${layer.ident}".`);
                }
                layerMap.set(layer.ident, layers.pg(layer.value));
                continue;
            }
            if (typeof layer.value !== "object" || layer.value === null) {
                throw new Error(`Pool metadata has invalid pg connection options for "${layer.ident}".`);
            }
        ...
        if (layer.type === "mysql") {
            if (layer.mode === "url") {
                if (typeof layer.value !== "string") {
                    throw new Error(`Pool metadata has invalid mysql connection string for "${layer.ident}".`);
                }
                layerMap.set(layer.ident, layers.mysql(layer.value));
                continue;
            }
            if (typeof layer.value !== "object" || layer.value === null) {
                throw new Error(`Pool metadata has invalid mysql connection options for "${layer.ident}".`);
            }
    ...
    for (const depot of hydrated.depots) {
        if (depot.type === "local") {
            if (depot.mode !== "local" || typeof depot.value !== "string") {
                throw new Error(`Pool metadata has invalid local depot config for "${depot.ident}".`);
            }
            depotMap.set(depot.ident, new LocalDepot(depot.value));
            continue;
        }
        if (depot.type === "s3") {
            if (depot.mode !== "s3" || typeof depot.value !== "object" || depot.value === null) {
                throw new Error(`Pool metadata has invalid s3 depot config for "${depot.ident}".`);
            }
    ...
    const options: PoolOptions & { backups?: { depotIdent: string; manager: BackMan } } = {};
    if (hydrated.wal) {
...
function normalizeResetMode(mode?: DangerResetMode): "all" | "layers" | "depots" | "meta" {
    if (!mode || mode === "all" || mode === "layers" || mode === "depots" || mode === "meta") {
        return mode ?? "all";
    }
    if (mode === "meta only") {
        return "meta";
...
async function reset(pool: LayerPool, options: DangerResetOptions = {}): Promise<void> {
    ...
    if (dropLayers) {
        for (const layer of layers.values()) {
            if (layer.type === "sqlite") {
                const sqlite = layer as unknown as { _db: { prepare: (sql: string) => { all: () => Array<{ name: string }> }; run: (sql: string) => void } };
                const tables = sqlite._db.prepare(
            ...
            if (layer.type === "pg") {
                const pg = layer as unknown as { _db: { unsafe: (sql: string, params?: unknown[]) => Promise<unknown> } };
                ...
                for (const row of rows) {
                    await pg._db.unsafe(`DROP TABLE IF EXISTS "${row.table_name}" CASCADE`);
                }
            ...
            if (layer.type === "mysql") {
                const mysql = layer as unknown as { _pool: { query: (sql: string, params?: unknown[]) => Promise<unknown> } };
...
const korm = {
     ...
     */
    file<T extends JSONable = JSONable>(params: FileParams<T>): FloatingDepotFile {
        const rn = typeof params.rn === "string" ? korm.rn<T>(params.rn) : params.rn;
    ...
    resolve<const Paths extends readonly string[]>(...paths: Paths): GetOpts<Paths> {
 ...
 * Pass each entry as `{ layer, ident? }` or `{ depot, ident? }`, then optional `{ walMode }`.
...
function pool(...entries: (PoolEntry | PoolOptions)[]): LayerPool | PoolBuilderStart {
    if (entries.length === 0) {
    ...
    for (const entry of input) {
        if (!isPoolEntry(entry)) {
            throw new Error("Invalid pool entry. Entries must be layers, depots, or a final options object.");
        }
    }
...
function isLayerInputEntry(value: LayerInput): value is { layer: SourceLayer; ident: string } {
    return Boolean(value && typeof value === "object" && "layer" in value);
}

function isDepotInputEntry(value: DepotInput): value is { depot: Depot; ident: string } {
    return Boolean(value && typeof value === "object" && "depot" in value);
}

function normalizeLayerInputs(entries: readonly LayerInput[]): Map<string, SourceLayer> {
    const layerMap = new Map<string, SourceLayer>();
...
function normalizeDepotInputs(entries: readonly DepotInput[]): Map<string, Depot> {
    const depotMap = new Map<string, Depot>();
    for (const entry of entries) {
...
function createPoolBuilder(): PoolBuilderStart {
    return {
        setLayers: <const Entries extends readonly LayerInput[]>(...entries: Entries) => {
            if (entries.length === 0) {
            ...
            return attachPoolBuilderMethods<LayerIdentFrom<Entries>, never>(context) as PoolBuilderWithLayers<LayerIdentFrom<Entries>>;
        },
    };
}
...
function attachPoolBuilderMethods<LIds extends string, DIds extends string>(
    ...
    const createCallableBuilder = (): RetainCallable<LIds, DIds> => {
        ...
        return new Proxy(callable, {
            get(_target, prop) {
                const value = (builder as unknown as Record<PropertyKey, unknown>)[prop];
    ...
    if (!mutable.withMeta) {
        mutable.withMeta = ((opts: PoolMetaModeFor<LIds>) => {
            ...
            if (!context.layers.has(opts.layerIdent)) {
                throw new Error(`Pool meta layer "${opts.layerIdent}" is not present in the pool.`);
            }
            context.metaMode = opts;
            return builder as PoolBuilderWithLayers<LIds> & PoolBuilderWithDepots<LIds, DIds>;
        }) as unknown as PoolBuilderWithDepots<LIds, DIds>["withMeta"];
    }

    if (!mutable.withLocks) {
        mutable.withLocks = ((opts: LockModeFor<LIds>) => {
            if (context.lockMode) {
                throw new Error("Shared locks are already configured for this pool.");
            }
            if (!context.layers.has(opts.layerIdent)) {
                throw new Error(`Lock layer "${opts.layerIdent}" is not present in the pool.`);
            }
            context.lockMode = opts;
            return builder as PoolBuilderWithLayers<LIds> & PoolBuilderWithDepots<LIds, DIds>;
    ...
    if (!mutable.withWal) {
        mutable.withWal = ((opts: WalModeFor<DIds>) => {
            if (context.depots.size === 0) {
                throw new Error("withWal(...) requires at least one depot. Call setDepots(...) first.");
            }
            if (context.walMode) {
                throw new Error("WAL is already configured for this pool.");
            }
            if (!context.depots.has(opts.depotIdent)) {
                throw new Error(`WAL depot "${opts.depotIdent}" is not present in the pool.`);
            }
    ...
    if (!mutable.backups) {
        mutable.backups = ((depotIdent?: DIds) => {
            if (context.depots.size === 0) {
                throw new Error("backups(...) requires at least one depot. Call setDepots(...) first.");
            }
            if (context.backupsManager) {
                if (depotIdent && depotIdent !== context.backupsDepotIdent) {
                    throw new Error(`Backups are already configured for depot "${context.backupsDepotIdent}".`);
                }
            } else {
                const chosenIdent = depotIdent ?? (context.depots.size === 1 ? [...context.depots.keys()][0] : undefined);
                if (!chosenIdent) {
                    throw new Error("backups(...) requires a depot ident when multiple depots are present.");
                }
                if (!context.depots.has(chosenIdent)) {
                    throw new Error(`Backups depot "${chosenIdent}" is not present in the pool.`);
                }
                const manager = new BackMan();
                manager.conveyLayers(context.layers).conveyDepot(context.depots.get(chosenIdent), chosenIdent);
            ...
            const backupsMutable = backupsBuilder as Partial<BackupsBuilder<LIds, DIds>>;
            if (!backupsMutable.addInterval) {
                backupsMutable.addInterval = ((layerIdent: LayerIdentOrWildcard<LIds>, interval: IntervalSpec) => {
                    if (!context.backupsManager) {
                        throw new Error("addInterval(...) requires backups(...) to be configured first.");
                    }
                    if (layerIdent !== "*" && !context.layers.has(layerIdent)) {
                        throw new Error(`Layer "${layerIdent}" is not present in the pool.`);
                    }
            ...
            if (!backupsMutable.retain) {
                backupsMutable.retain = ((policy: "all" | "none" | number) => {
                    if (!context.backupsManager) {
                        throw new Error("retain(...) requires backups(...) to be configured first.");
                    }
                    if (!context.lastBackupLayer) {
                        throw new Error("retain(...) must follow addInterval(...) to know which layer to target.");
                    }
                    const layerIdent = context.lastBackupLayer;
                    if (policy === "all" || policy === "none") {
                        context.backupsManager.setRetentionForLayer(layerIdent, { mode: policy });
                        return backupsBuilder;
                    }
                    const selector = {
                        get backups() {
                            context.backupsManager!.setRetentionForLayer(layerIdent, { mode: "count", count: policy });
                            return createCallableBuilder();
                        },
                        get days() {
                            context.backupsManager!.setRetentionForLayer(layerIdent, { mode: "days", days: policy });
                            return createCallableBuilder();
                        },
                    };
