...
export class Encrypted<T extends JSONable> {
  ...
  constructor(params: {
    ...
    if (params.type === "symmetric" && (!params.iv || !params.authTag)) {
      throw new Error(
        "FATAL: IV and authTag are required for symmetric encryption",
      );
    }
  ...
  private async _verifyPassword(
    ...
    if (this.type !== "password") {
      throw new Error(
        "FATAL: Password verification is only supported for password encryption.",
      );
    }
    try {
  ...
  async verifyPassword(plainText: string): Promise<boolean> {
    if (this.type !== "password") {
      throw new Error(
        "FATAL: Password verification is only supported for password encryption. You can decrypt the value with `decrypt(): T` instead.",
      );
    }
...
export class Encrypt<T extends JSONable> {
  ...
  static fromEncryptedPayload<T extends JSONable>(
    ...
    if (payload.type === "password") {
      ...
      if (plainValue === undefined) {
        throw new Error(
          "FATAL: Cannot hydrate symmetric encryption without plaintext.",
        );
      }
  ...
  toJSON(): JSONable {
    if (!this._encryptedValue) {
      throw new Error(
        "FATAL: Encrypted value is not available. You must call lock() first.",
      );
    }
  ...
  async verifyPassword(plainText: string): Promise<boolean> {
    ...
    return await this._encryptedValue!.verifyPassword(
    ...
    if (this._type === "symmetric") {
      if (!process.env.KORM_ENCRYPTION_KEY) {
        if (process.env.NODE_ENV === "production") {
          throw new Error(
            "FATAL: KORM_ENCRYPTION_KEY is missing in PRODUCTION. Aborting to prevent data loss.",
          );
        }
        const newKey = randomBytes(32).toString("hex");
        process.env.KORM_ENCRYPTION_KEY = newKey;
        console.warn(
          "\x1b[31m!!! READ THIS !!!\x1b[0m -> Wanted to symmetrically encrypt a value but the KORM_ENCRYPTION_KEY environment variable is not set.",
        );
        console.warn(
          "korm has generated a new encryption key for you. It will stay valid for this session.",
        );
        console.warn(
          "Do not share this key with anyone. It can be used to decrypt the data stored in your database.",
        );
        console.warn(
          "You must save this key in a secure location to access data encrypted with it.",
        );
        console.warn(
          "To use the same key next session, set the KORM_ENCRYPTION_KEY environment variable to the value of the new key.",
        );
        console.log(
      ...
      const encryptedValue = await this._hashPassword(
...
/**
 * Convenience alias for password-hash fields in item data.
 * Create values via `korm.password(...)`.
 */
 ...
 */
export async function decrypt<T extends JSONable>(
  encrypted: EncryptedParams<T>,
): Promise<T> {
  if (encrypted.type !== "symmetric") {
    throw new Error(
      "FATAL: Decryption is only supported for symmetric encryption.",
    );
  }
  if (!process.env.KORM_ENCRYPTION_KEY) {
