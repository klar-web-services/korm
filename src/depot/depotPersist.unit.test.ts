...
import {
  persistDepotFilesInData,
  uploadDepotFilesFromRefs,
} from "./depotPersist";
...
describe("depotPersist", () => {
  ...
  test("rejects uploads when RN is not a depot", async () => {
    ...
    await expect(
      persistDepotFilesInData({ bad: badFile } as JSONable, pool),
    ).rejects.toThrow("does not refer to a depot");
  });
  ...
  test("rejects uploads when RN is a depot prefix", async () => {
    ...
    await expect(
      persistDepotFilesInData({ bad: badFile } as JSONable, pool),
    ).rejects.toThrow("Expected depot file RN");
  });
