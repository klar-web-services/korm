 ...
 * Use `korm.depot.s3(...)` to create and include in a pool.
 */
export class S3Depot implements Depot {
    ...
    constructor(options: S3DepotOptions) {
        ...
        this._endpoint = options.endpoint ?? process.env.S3_ENDPOINT ?? process.env.AWS_ENDPOINT;
        this._region = options.region ?? process.env.S3_REGION ?? process.env.AWS_REGION ?? "us-east-1";
        this._accessKeyId = options.accessKeyId ?? process.env.S3_ACCESS_KEY_ID ?? process.env.AWS_ACCESS_KEY_ID;
        this._secretAccessKey = options.secretAccessKey ?? process.env.S3_SECRET_ACCESS_KEY ?? process.env.AWS_SECRET_ACCESS_KEY;
        this._sessionToken = options.sessionToken ?? process.env.S3_SESSION_TOKEN ?? process.env.AWS_SESSION_TOKEN;
    ...
    private _resolveEndpoint(): URL {
        if (this._endpoint) {
            ...
            return new URL(hasProtocol ? this._endpoint : `https://${this._endpoint}`);
        }
    ...
    private async _createBucket(): Promise<void> {
        ...
        throw new Error(`Failed to create bucket "${this._bucket}": ${response.status} ${response.statusText} ${text}`);
    }
    ...
    async listDirs(rn: RN): Promise<string[]> {
        ...
        await this._ensureBucket();
        const listPrefix = this._prefixForList(rn);
        const result = await this._client.list({ prefix: listPrefix, delimiter: ":" });
        ...
        return prefixes.map((entry) => {
            ...
            const relative = stripped.startsWith(base) ? stripped.slice(base.length) : stripped;
            return relative.replace(/:$/, "");
        });
    }

    /** @inheritdoc */
    async editFile(rn: RN, edit: (file: DepotFile) => Promise<DepotFileBase>): Promise<boolean> {
        if (rn.pointsTo() !== "depotFile") {
            throw new Error(`Expected depot file RN but got "${rn.value()}".`);
        }
        await this._ensureBucket();
