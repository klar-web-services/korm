 ...
 * Use `korm.depot.local(rootPath)` to create and include in a pool.
 */
export class LocalDepot implements Depot {
    ...
    private _pathFromRn(rn: RN): string {
        ...
        const normalized = parts[parts.length - 1] === "*" ? parts.slice(0, -1) : parts;
        const base = path.resolve(this._root);
        const resolved = normalized.length === 0 ? base : path.resolve(base, ...normalized);
