...
describe("DepotFile", () => {
    ...
    test("throws when slicing unsupported payloads", () => {
        ...
        expect(() => file.slice(0, 1)).toThrow("DepotFile.slice() is not supported");
    });
    ...
    test("accepts edits returning depot files and validates RN", async () => {
        ...
        const other = RN.create("[rn][depot::files]:invoices:2024:other.txt").unwrap();
    ...
    test("validates RN type on create and commit", async () => {
        ...
        await expect(floating.create(pool)).rejects.toThrow("Expected depot file RN");
        const uncommitted = new UncommittedDepotFile(rn, new Blob(["y"]));
        await expect(uncommitted.commit(pool)).rejects.toThrow("Expected depot file RN");
    });
