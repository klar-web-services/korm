...
/** A WAL operation entry for insert/update/delete. */
export type WalOp =
  | {
      /** Operation type. */
      type: "insert" | "update";
  ...
  | {
      ...
      /** RN string for the target item. */
      rn: string;
      /** Optional placeholder payload (unused). */
      data?: null;
      /** Optional before-image for undo. */
      before?: JSONable | null;
      /** When true, allow destructive schema changes. */
...
/** WAL operation describing a depot file write/delete. */
export type WalDepotOp =
  | {
      /** Operation type for depot writes. */
      type: "put";
  ...
  | {
      /** Operation type for depot deletes. */
      type: "delete";
      /** RN string for the depot file. */
      rn: string;
      /** Optional RN for the previous payload, if any. */
...
/** WAL staging input for depot operations. */
export type WalDepotStageOp =
  | DepotFileBase
  | {
      /** Operation type for depot deletes. */
      type: "delete";
      /** RN for the depot file to remove. */
      rn: RN<JSONable>;
    };
...
function isDepotDeleteOp(
  op: WalDepotStageOp,
): op is { type: "delete"; rn: RN<JSONable> } {
  return (op as { type?: string }).type === "delete";
}
...
export class WalManager {
  ...
  constructor(access: WalPoolAccess, options: WalMode) {
    ...
    if (!depot) {
      throw new Error(
        `WAL depot "${this._depotIdent}" is not present in the pool.`,
      );
    }
    this._depot = depot;
    this._poolId = computePoolId(access.getLayers(), access.getDepots());
    const namespace = options.walNamespace
      ? normalizeSegment(options.walNamespace, "walNamespace")
      : this._poolId;
  ...
  private async _stageDepotOps(
    depotOps: WalDepotStageOp[],
    recordId: string,
  ): Promise<WalDepotOp[]> {
    ...
    try {
      for (let index = 0; index < depotOps.length; index++) {
        const op = depotOps[index]!;
        if (isDepotDeleteOp(op)) {
          const rn = op.rn;
          if (!rn.isDepot() || rn.pointsTo() !== "depotFile") {
            throw new Error(
              `WAL depot op RN "${rn.value()}" is not a depot file.`,
            );
          }
      ...
      if (createdPayloads.length > 0) {
        await Promise.all(
          createdPayloads.map((created) => this._deletePayload(created)),
        );
      }
      throw error;
    }
   ...
   */
  async stage(ops: WalOp[], depotOps?: WalDepotStageOp[]): Promise<WalHandle> {
    ...
    if (depotOps && depotOps.length > 0) {
      if (!this.depotOpsEnabled) {
        throw new Error(
          'WAL depot ops are disabled. Set walMode.depotOps to "record".',
        );
      }
    ...
    const record: WalRecord = {
      ...
      ops: opsWithBefore,
      depotOps:
        stagedDepotOps && stagedDepotOps.length > 0
          ? stagedDepotOps
          : undefined,
    };
    ...
    for (const op of record.ops) {
      ...
      if (op.type === "insert") {
        ...
        if (!result.success && !isDuplicateError(result.error)) {
          throw (
            result.error ??
            new Error(`Failed WAL insert for RN "${rn.value()}"`)
          );
        }
        continue;
      }
      if (op.type === "delete") {
        const result = await layer.deleteItem(rn, {
          destructive: op.destructive,
        });
        if (!result.success && !isMissingItemError(result.error)) {
          throw (
            result.error ??
            new Error(`Failed WAL delete for RN "${rn.value()}"`)
          );
        }
        continue;
      }
      ...
      if (!result.success) {
        throw (
          result.error ?? new Error(`Failed WAL update for RN "${rn.value()}"`)
        );
      }
    }
  }
    ...
    for (const op of ops) {
      ...
      if (op.type === "delete") {
        ...
        if (!result.success && !isDuplicateError(result.error)) {
          throw (
            result.error ??
            new Error(`Failed WAL undo delete for RN "${rn.value()}"`)
          );
        }
        continue;
      }
      ...
      if (!result.success) {
        throw (
          result.error ??
          new Error(`Failed WAL undo update for RN "${rn.value()}"`)
        );
      }
    }
  }
  ...
  async recoverPending(): Promise<void> {
    ...
    for (const file of files) {
      ...
      try {
        ...
        throw new Error(
          `Failed to parse WAL record "${file.rn.value()}": ${error}`,
        );
      }
